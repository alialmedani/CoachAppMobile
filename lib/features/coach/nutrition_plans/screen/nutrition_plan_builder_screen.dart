...
class _NutritionPlanBuilderScreenState
  ...
  void initState() {
    ...
    _targetCalories = TextEditingController(text: _fmtTarget(plan?.targetCalories));
    _targetProtein = TextEditingController(text: _fmtTarget(plan?.targetProteinG));
    ...
    _meals = plan != null
        ? List<MealModel>.from(plan.meals)
        : <MealModel>[];
  ...
  double get _estProtein =>
      _meals.fold(0.0, (sum, m) => sum + m.totalProteinG);
  ...
  String? _syncAndValidate(NutritionPlanCubit cubit) {
    ...
    for (final c in [_targetCalories, _targetProtein, _targetCarbs, _targetFat]) {
