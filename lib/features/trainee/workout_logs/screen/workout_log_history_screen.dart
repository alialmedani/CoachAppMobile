...
import 'workout_log_view_screen.dart';

/// Paged history of the trainee's own workout logs (newest first). Rows are
...
///
/// Requires a [WorkoutLogCubit] provided by the caller.
class WorkoutLogHistoryScreen extends StatefulWidget {
  const WorkoutLogHistoryScreen({super.key});

  @override
  State<WorkoutLogHistoryScreen> createState() =>
      _WorkoutLogHistoryScreenState();
}

class _WorkoutLogHistoryScreenState extends State<WorkoutLogHistoryScreen> {
  ...
  Future<void> _pickFrom(WorkoutLogCubit cubit) async {
  ...
  Future<void> _pickTo(WorkoutLogCubit cubit) async {
    final picked = await showDatePicker(
      context: context,
  ...
  void _clearDates(WorkoutLogCubit cubit) {
    setState(() {
      _from = null;
      _to = null;
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppTopBar(title: 'workout_history'.tr()),
      body: Column(
        children: [
          Container(
          ...
          Expanded(
            child: PaginationList<WorkoutLogModel>(
              withPagination: true,
              onCubitCreated: (c) => _pagination = c,
              repositoryCallBack: (data) => cubit.fetchWorkoutLogList(data),
              noDataWidget: AppEmptyState(
                icon: Icons.fitness_center_outlined,
                title: 'no_workout_logs'.tr(),
                subtitle: 'no_workout_logs_subtitle'.tr(),
  ...
  Future<void> _openDetail(WorkoutLogCubit cubit, WorkoutLogModel log) async {
    final changed = await Navigator.push<bool>(
      context,
      MaterialPageRoute(
        builder: (_) => BlocProvider.value(
          value: cubit,
          child: WorkoutLogViewScreen(logId: log.id ?? ''),
        ),
      ),
    );
